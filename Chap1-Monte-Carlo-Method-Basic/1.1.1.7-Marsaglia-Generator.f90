program main
    ...
    interface
        function Marsaglia_generator(seed_1, seed_2)
            implicit none
...
function Marsaglia_generator(seed_1, seed_2)
    ...
    if (present(seed_1)) then
        if (present(seed_2)) then
            Marsaglia_generator = Lagged_Fibonacci_generator(seed_1) - random_generator_2(seed_2)
        else
            Marsaglia_generator = Lagged_Fibonacci_generator(seed_1) - random_generator_2()
        end if
    else
        if (present(seed_2)) then
            Marsaglia_generator = Lagged_Fibonacci_generator() - random_generator_2(seed_2)
        else
    ...
    contains
        function Lagged_Fibonacci_generator(seed)
        ...
        function random_generator_2(seed)
            ! 第二个随机数发生器
            implicit none
            integer, parameter :: li = selected_int_kind(10)
            integer, parameter :: dp = selected_real_kind(8)
